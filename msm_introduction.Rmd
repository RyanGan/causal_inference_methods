...
boot_msm <- simdata %>% bootstrap(1000) %>% 
  do(tidy(MSModelR::g.formula(data = ., y = "Y", a = "A", q.model = "A+W1+A:W2",
            model.family = "gaussian", estimate = "diff")))
# iptw
set.seed(123)
boot_iptw <- simdata %>% bootstrap(1000) %>% 
  do(tidy(MSModelR::iptw(data = ., y="Y",a="A",w="W1+W2",estimate="diff")))
...
estimates[1,2] <- round(quantile(boot_msm$x, 0.5),2)
estimates[1,3] <- round(quantile(boot_msm$x, 0.025),2)
estimates[1,4] <- round(quantile(boot_msm$x, 0.975),2)

# fill matrix for iptw
estimates[2,1] <- "IPTW"
estimates[2,2] <- round(quantile(boot_iptw$x, 0.5),2)
estimates[2,3] <- round(quantile(boot_iptw$x, 0.025),2)
estimates[2,4] <- round(quantile(boot_iptw$x, 0.975),2)
