...
mediation <- function(data, y, x, m, c, y_family, m_family){
  ...
  y_on_m <- cde_mod$coefficients[3]
  x_on_m <- m_x_mod$coefficients[2]
 
  estimates <- cbind(total_effect, cde, nie, x_on_m, y_on_m)
  return(estimates)
...
# fill m x relationship
est[4,1] <- "x_m_association"
est[4,2] <- round(exp(quantile(boot_mediation$x_affect_on_m, 0.5)),3)
est[4,3] <- round(exp(quantile(boot_mediation$x_affect_on_m, 0.025)),3)
est[4,4] <- round(exp(quantile(boot_mediation$x_affect_on_m, 0.975)),3)
